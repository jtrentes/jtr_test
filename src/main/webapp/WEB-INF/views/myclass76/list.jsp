...
<h1>List MyClass76</h1>
<bR />
<form:form class="form-horizontal" method="POST" modelAttribute="myClass76Criteria">
	<div class="row">
		...
		<c:set var="myclass89error">
			<form:errors path="myclass89" />
			
		</c:set>
		<div class="control-group <c:if test="${not empty myclass89error}">error</c:if> span6">
			<label class="control-label" for="myclass89input">myclass89</label>
			<div class="controls">
				<form:select id="myclass89input" path="myclass89.id">
					<form:options items="${myClass89s}" itemValue="id" itemLabel="id" />
					<form:option value="0" label="------------------" />
					
				</form:select>
				<form:errors class="help-inline" path="myclass89" />
...
<table class="table table-striped table-bordered table-condensed">
	<tr>
		...
		<th>id</th>
		<th>
			<a class="btn btn-primary pull-right" href="<c:url value="/myclass76/edit"/>">New</a>
			
		</th>
		
	</tr>
	<c:forEach items="${myClass76List}" var="myClass76">
		<tr>
			<td>${myClass76.attribute}</td>
			<td>${myClass76.attribute2}</td>
			<td>${myClass76.attribute3}</td>
			<td>${myClass76.attribute4}</td>
			<td>${myClass76.attribute5}</td>
			<td>${myClass76.attribute6}</td>
			<td>${myClass76.attribute7}</td>
			<td>${myClass76.id}</td>
			<td>
				<a class="btn" href="<c:url value="/myclass76/detail/${myClass76.id}"/>">View</a>
				<a class="btn" href="<c:url value="/myclass76/edit/${myClass76.id}"/>">Edit</a>
